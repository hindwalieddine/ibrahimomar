...
{
	$seq['citizen']=array();
	$seq['citizen_to_realty']=array('citizen_id');
}
...
?>
	<div id="body">	
		...
		<table >
		<!------------------------------ Customs ----------------------------->
			<tr ><td><span ><a href="addCitizen_to_realty.php?mod=1">Create Citizen and Add To Realty</a></span></td></tr>
		<!-------------------------------------------------------------------->		
		<?php
				...
				<?php
									...
									foreach($foreignTable as $id2=>$item)
									{
										...
										if($Ftable!='citizen')
										{
											...
											//$FDN=$index->GetFullDisplayName($item[$PRI],$Ftable);
										}
										//********** customs *******************
										$sel=''; if(isset($_GET['realty_id']) && $id=='realty_id' && $item[$PRI]==$_GET['realty_id']) $sel='selected';
										//*************************************
										if($Ftable!='citizen')
										{
											echo('<option '.$sel.' value="'.$item[$PRI].'" >'.$FDN.'</option>');
										}
										else
										{
											echo('<option '.$sel.' value="'.$item[$PRI].'" >'.$item['name'].' - '.$item['id'].'</option>');
										}
									}
								}
			?>
